...
export const ActorController = {
    ...
    update: async (id: number, body: { first_name: string; last_name: string }) => {
        try {
            ...
            return HttpResponse.ok(updatedActor, "Actor actualizado");
        } catch (error) {
    ...
    remove: async (id: number) => {
        try {
            const deletedActor = await ActorService.remove(id);
            return HttpResponse.ok(deletedActor, "Actor eliminado");
        } catch (error) {
